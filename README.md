...
- 支援前四碼、不拘、後六碼、尾數反查、特殊號碼費、各位置不含 4
- 後六碼查詢用 `x` 代表任意數字，例如 `58xx58`
- 尾數反查可直接輸入後 `2 / 3 / 4 / 5 / 6` 碼，或像 `56xx56` 這種最多 `4` 個 `x` 的寫法，跨全部官方 `09` 前綴合併結果
- 若尾數條件展開後達到 `4` 個 wildcard，會先抓每個前綴第 `1` 頁，避免一次對官方站打太重
- 4 個 `x` 會自動拆成 10 組官方允許的查詢並合併結果
- 5 個以上 wildcard 不支援，避免過度查詢官方站
- 可選擇抓 1、3、5 頁官方結果，每頁 20 筆
- 可分享查詢條件連結，打開後會自動還原條件
- 可分享完整工作台連結，一次帶上查詢條件與收藏清單
- 桌機分享會自動顯示短版連結與 QR，手機則沿用原生分享
- 分享視窗可直接下載 QR PNG
- 支援把 QR 直接複製成圖片；不支援時會退回下載 PNG
- 分享視窗可一鍵複製純文字門號清單
- 分享摘要 chips 可直接點按複製，例如前四碼組合或後六碼條件

### 結果體驗

- 查詢結果可切換一排 1 個 / 一排 2 個
- 可依好記度或號碼排序
- 可收藏待選門號，支援一鍵複製、分享連結、匯出、匯入
